...
export default function ServerInfoDialog({ instance, server, ...control }: DialogControl & { instance: Instance | undefined; server: ServerInfo | undefined }) {
	return (
			...
			{instance && instance.deletedAt === null && server ? (
				<DataListKv
					data={{
						...
						在线玩家: server.running ? server.onlinePlayers.length : 0,
						版本: server.running ? server.data.version.name.clean : '-'
					}}
				></DataListKv>
			) : (
				<NoInstance/>
